...
У аккаунта одна роль, она задаётся при создании и не меняется. Через
регистрацию появляется только заказчик. Аккаунт эксперта создаёт админ,
когда одобряет заявку, поэтому email эксперта должен быть свободен.
...
Админ правит эксперта через `PATCH /admin/experts/{user_id}` (имя, телефон,
направления) и `PATCH|DELETE /admin/experts/{user_id}/certificates/{id}`.
`DELETE /admin/experts/{user_id}` удаляет профиль вместе с аккаунтом.
...
Каждый шаг экспертизы кладёт другой стороне строку в `notifications` — это
делают сами сценарии через `NotificationRepository.add_all` в той же
транзакции. Письмо на почту уходит отдельно, фоновой задачей роутера.

`GET /notifications` — список, `POST /notifications/{id}/read` — одно,
`POST /notifications/read-all` — все. Автоматически ничего не читается:
только по кнопке в кабинете, счётчик непрочитанных висит на вкладке
«Уведомления».
...
Чтобы админ не проверял банк вслепую, заказчик жмёт «Я оплатил»:
`POST /invoices/{id}/paid` ставит `reported_at` и шлёт письмо на
`NOTIFY_EMAILS`. Это только сигнал — статус экспертизы по нему не двигается,
метка видна в кабинете заказчика и в админке рядом со счётом.

| Ручка | Что отдаёт |
|---|---|
| `GET /invoices` | счета заказчика |
| `POST /invoices/{id}/paid` | заказчик сообщает, что оплатил счёт |
