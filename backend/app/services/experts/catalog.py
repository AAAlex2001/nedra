...
Технические устройства и здания подключим позже.

Категория эксперта по классу опасности ОПО: эксперт первой категории
работает с объектами любого класса, второй — со II, III и IV, третьей —
с III и IV. Поэтому для заявки по классу считаем минимально достаточную
категорию, а подходят эксперты с этой категорией и выше.
...
OBJECTS: list[ExpertiseObject] = [
    ...
    ExpertiseObject(
        "kl_tp",
        "КЛ/ТП",
        "Документация на консервацию, ликвидацию и техническое перевооружение",
        "Единое удостоверение на проекты консервации, ликвидации и технического "
        "перевооружения для отраслей, где Ростехнадзор не разделяет эти виды документации.",
    ),
    ExpertiseObject(
        "d",
        "Д",
...
SPLIT_DOCS = ("kl", "tp", "d", "ob")
JOINED_DOCS_WITH_DECLARATION = ("kl_tp", "d", "ob")
JOINED_DOCS_NO_DECLARATION = ("kl_tp", "ob")

HAZARD_CLASSES = (1, 2, 3, 4)

HAZARD_CLASS_CATEGORY = {1: 1, 2: 2, 3: 3, 4: 3}
